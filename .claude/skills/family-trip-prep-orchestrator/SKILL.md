...
| T06b-notion-sync | Orchestrator (서브에이전트에게 위임하지 않음) | `02-itinerary-current.md`·`03-checklist-review.md` 최신 데이터 | 노션 "뉴욕 여행 — 한장 정리" 페이지 + "봉봉겨울뉴욕여행"(정리판) 페이지(91차부터 편입) | T06 | 두 노션 페이지 모두 예산 표·일자별 일정·조건부 사항 콜아웃이 최신 데이터와 일치("한장 정리"는 제목·상단 안내문 버전 표기도 갱신, "봉봉겨울뉴욕여행"은 회차 태그 없이 값만 갱신하고 개인정보·실사용 메모 블록은 절대 건드리지 않음) | 시작, 차단, 완료 |
...
#### 두 번째 노션 페이지 "봉봉겨울뉴욕여행"(정리판)도 91차부터 동기화 대상에 편입(2026-09-13)

2026-09-12에 "회차 태그 없이 깔끔하게 보여달라"는 요청으로 별도 생성했던 정리판 페이지(`https://app.notion.com/p/3d9f32caad3281f9b89cd217f8a653e0`)가 원래는 T03~T07 자동 파이프라인 대상이 아니었다. 그런데 91차에서 사용자가 이 페이지를 실제 작업 문서로 쓰고 있다는 것이 드러났다 — 계정 정보·여권 정보·준비물·실제 예약 확인 내역(알라딘 실결제가 728,418원으로 배정가 400,000원과 다름 등)을 이 페이지에 직접 채워 넣은 상태였고, 일정·예산은 87차(v43) 시점에 멈춰 있어 사용자가 "노션 업데이트 안 한 거 아니냐"고 지적했다. 사용자 확인 후(91차) **이 페이지도 매 회차 자동 동기화 대상으로 편입**한다.

- **위 "뉴욕 여행 — 한장 정리"와 동일하게 `update_content`만 사용, `replace_content` 절대 금지** — 이유는 같다(사용자가 직접 편집한 내용 보존).
- **⚠⚠ 이 페이지는 "한장 정리"보다 개인정보·실사용 메모 비중이 훨씬 높다.** `## 일자별 일정` 토글 바로 아래, Day 1 앞에 있는 `💜계정❌`·`💜준비물❌`·`💜 유심 로밍 도깨비`·`💚투어 예매 내역❌`·`💚맛집예약 ❌`·`💚 이동수단 ❌`·`💙 여권 정보 ❌`·`✈️ 아시아나항공`·`⛺ 숙소 - 에어비엔비` 블록들(계정 ID·비밀번호·여권 정보·실제 예약번호·결제 계좌 등)은 **T06b가 절대 건드리지 않는다** — 이 블록들은 정본(`02-itinerary-current.md`)에 대응하는 데이터가 아예 없고, 오직 사용자가 손으로만 채워 넣는다.
- **갱신 대상은 예산 배분 표·"지금 잡아야 할 것" 콜아웃의 예약 액션 목록(체크되지 않은 항목만 텍스트 교체, 이미 체크된 항목은 사용자의 실제 진행 상황이므로 손대지 않음)·일자별 일정의 시각표 줄(체크박스 상태는 유지, 시각·활동명만 텍스트 교체)뿐**이다. 정본 개정 이력의 `[N차]` 태그는 이 페이지에 옮기지 않는다(이 페이지 자체 설계가 "회차 태그 없는 깔끔한 버전").
- 일정 줄을 교체할 때 **사용자가 직접 붙여 둔 개인 메모(`???`·`??` 물음표, "찾기??" 같은 자기 질문, 특정 메뉴 요청 등)는 반드시 그대로 옮겨 붙인다** — 91차에서 Day 1·Day 2·Day 9 재구성 시 이렇게 처리했다(예: "13:30 숙소 인근 마트나 밥…???→ 근처마트 찾기??" 문구를 시간만 그대로 두고 보존).
- 노션 마크다운은 대괄호 `[...]`가 리터럴로 저장될 때 이스코프 백슬래시(`\[`·`\]`)가 함께 저장되는 경우가 있다 — old_str이 매칭 안 되면 재-fetch로 정확한 저장 형태(백슬래시 포함 여부)를 확인하고 다시 시도한다(91차에서 실제로 이 문제로 한 번 실패했다).
...
12. **노션 동기화(T06b, Orchestrator 직접 수행)**: 위 "T06b-notion-sync" 절 지시대로 노션 "한장 정리" 페이지와 "봉봉겨울뉴욕여행"(정리판) 페이지를 둘 다 이번 회차 데이터로 갱신한다("봉봉겨울뉴욕여행"은 개인정보·실사용 메모 블록 제외, 91차부터 편입). 페이지가 아직 없으면(최초 실행) 새로 만들고 URL을 `README.md`에 기록하며, 있으면 항상 같은 URL로 갱신한다.
...
**⚠ 정본을 한 줄이라도 편집했으면 표면 4곳 반영 확인(2026-09-04 58차 신설, 2026-09-13 91차에 노션 페이지 1곳 추가)**: `02-itinerary-current.md`를 이번 회차에 편집했다면 — T03 정식 재실행이 아니라 "값 하나 노출"·"메모 한 줄 추가" 수준이어도 — 그 편집이 `dashboard.html`·`dashboard-artifact.html`·노션 "한장 정리"·노션 "봉봉겨울뉴욕여행"(정리판) 네 곳에 반영됐는지 확인하고 T07을 닫는다. 56차가 정본 Day 9 메모만 고치고(`git stat: 1 file changed`) 대시보드·노션을 건너뛰어, 정작 "사용자가 볼 수 있게 노출"하려던 「1시간 여유」 노트가 58차까지 사용자 표면 어디에도 없었다. 기존 완료 기준은 "T03까지 갔으면 T06b 확인"만 명시해 **"T03 없이 정본 메모만 고친 회차"가 사각지대**였다.
